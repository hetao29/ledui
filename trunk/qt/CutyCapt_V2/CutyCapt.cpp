  ...
  switch (mFormat) {
    ...
    case Printer: {
      ...
      for (int i = 0; i < allPrinters.size(); ++i) {
	      qDebug() << "Printer "<<i<<" name is"<< allPrinters.at(i).printerName();
      }
...
CaptHelp(void) {
  printf("%s",
    ...
    "  --out=<path|printer>           The target file (.png|pdf|ps|svg|jpeg|...|prt)\n"
    "  --out-format=<f>               Like extension in --out, overrides heuristic \n"
//  "  --out-quality=<int>            Output format quality from 1 to 100          \n"
    "  --min-width=<int>              Minimal width for the image (default: 800)   \n"
    "  --min-height=<int>             Minimal height for the image (default: 600)  \n"
    "  --page-width=<int>             Minimal height for the image (default: 165)(hetal added)  \n"
    "  --page-height=<int>             Minimal height for the image (default: 102)(hetal added)  \n"
...
//  "  --user-styles=<url>            Location of user style sheet (deprecated)    \n"
    ...
    " -----------------------------------------------------------------------------\n"
    " -----------------------------------------------------------------------------\n"
    "  <f> is prt (is Printer Type, output must is printer,add by hetal)!!!!!!!    \n"
    " -----------------------------------------------------------------------------\n"
#if CUTYCAPT_SCRIPT
...
main(int argc, char *argv[]) {
  ...
  int argPageWidth = 165;
  int argPageHeight = 102;
